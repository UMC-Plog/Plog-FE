...
function ProjectMemberRow({ member }: { member: ProjectMember }) {
  return (
    <li className="flex min-h-14 items-center gap-3 py-2">
      ...
      <span className="min-w-0 flex-1 truncate text-body font-medium text-gray-900">
        {member.nickname}
      </span>
    </li>
  );
...
export function ProjectSettingsPage() {
  ...
  const nameError =
    nameTouched && !nameValid
      ...
      : undefined;
  const dateError =
    dateTouched && dateChanged && !dateValid
      ? "오늘보다 이후 날짜를 선택해 주세요"
      : undefined;
    ...
    <Layout className="bg-gray-25">
      ...
      <main className="flex-1 px-5 py-6">
        <form onSubmit={handleSubmit} noValidate>
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <h2 id="project-info-title" className="text-title font-bold text-gray-900">
                프로젝트 정보
              </h2>
            </div>
            ...
            <div className="mt-2 min-h-5" aria-live="polite">
              {saved && (
                <p className="flex items-center justify-center gap-1 text-caption font-normal text-success">
                  <Check size={14} aria-hidden="true" /> 변경사항을 저장했어요
                </p>
              )}
            </div>
          </section>
        ...
        <section
          ...
          className="mt-5 rounded-lg border border-gray-100 bg-white p-5 shadow-md"
        >
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <Users size={20} className="text-aqua-600" aria-hidden="true" />
              <h2 id="project-members-title" className="text-title font-bold text-gray-900">
                현재 팀원
              </h2>
            </div>
            <span className="rounded-full bg-aqua-50 px-3 py-1 text-caption text-aqua-700">
          ...
          {project.members.length > 0 ? (
            ...
            <p className="mt-4 rounded-md bg-gray-50 px-4 py-5 text-center text-body-sm text-gray-400">
              표시할 팀원이 없어요
            </p>
          )}
        </section>
